...
namespace ErGui {
	...
	bool DragFloat(const char* label, float* v, float step, float step_fast, const char* format, float v_speed, float v_min, float v_max, ImGuiSliderFlags flags, bool isMixed, std::optional<ImVec4> color) {
	...
	bool DragInt(const char* label, int* v, int step, int step_fast, const char* format, float v_speed, int v_min, int v_max, ImGuiSliderFlags flags, bool isMixed, std::optional<ImVec4> color) {
		const float button_size = ImGui::GetFrameHeight();
		bool value_changed = false;
		auto style = ImGui::GetStyle();
	...
	bool DragInt(const char* label, short* v, int step, int step_fast, const char* format, float v_speed, int v_min, int v_max, ImGuiSliderFlags flags, bool isMixed, std::optional<ImVec4> color) {
		int tempV = *v;
		bool result = DragInt(label, &tempV, step, step_fast, format, v_speed, v_min, v_max, flags, isMixed, color);
		if (result)
			*v = static_cast<short>(tempV);
		return result;
	}
