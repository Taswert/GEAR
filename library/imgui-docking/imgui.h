...
namespace ImGui
{
    ...
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), const float rounding = 0.f); // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
    IMGUI_API bool          Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0), const float rounding = 0.f);      // "bool* p_selected" point to the selection state (read-write), as a convenient helper.
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_DisableWheelRot = 1 << 29,  // [Picker]     // ColorPicker: disables rotation of color triangle. Taswert modification.
