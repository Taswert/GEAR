...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive() && !is_readonly)
        {
            ...
            if (flags & ImGuiColorEditFlags_DisableWheelRot) {
                cos_hue_angle = ImCos(0 * 2.0f * IM_PI);
                sin_hue_angle = ImSin(0 * 2.0f * IM_PI);
            }
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        draw_list->AddTriangle(tra, trb, trc, IM_COL32(0, 0, 0, 0), 0.f); //tra, trb, trc, col_midgrey, 1.5f
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, const float rounding)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (is_visible)
    {
        ...
        if (highlighted || selected)
        {
            ...
            RenderFrame(bb.Min, bb.Max, col, false, rounding);
        }
        if (g.NavId == id)
        {
...
bool ImGui::Selectable(const char* label, bool* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg, const float rounding)
{
    if (Selectable(label, *p_selected, flags, size_arg, rounding))
    {
